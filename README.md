...
| `dbt/` | dbt-duckdb project. Builds the `raw` → `transformed` → `gold` star schema from the live event CSV. Outputs `ultimate.duckdb` plus gzipped CSVs Power BI can ingest. |
| `infra/` | Bicep template for the Azure side. `azd up` provisions everything. |
