...
10. **Half Time / End Game** — currently auto-emitted at the configured thresholds; design direction is to surface a confirmation prompt before emitting (Phase 3 of the build log). At Half Time, possession goes to the team that did not start the game (opposite of the first pull).
